...
> 家常菜谱库 + 冰箱管理 + 智能计划 + 购物清单 + 饭团游戏化 + 家庭空间 + 家庭同步。饭团 AI 支持用户自配 OpenAI-compatible Key，健康事实可由对话提取并写入本地档案。
...
| 功能 | 说明 |
|------|------|
| 🍳 菜品推荐 | 内置家常菜谱，按冰箱食材、健康标签、家庭偏好智能排序 |
...
> 普通家庭请直接使用上方在线体验地址；家庭成员只需网址和邀请码，不需要下载、部署或维护代码。详见 [家庭使用与更新](./docs/家庭使用与更新.md)。

### 维护者发布（仅官方仓库）

`auto-publish` 会把工作区改动直接推送到 `main`，仅适用于已完成验证、准备发布的改动；功能施工期间必须停用它，避免半成品上线。
